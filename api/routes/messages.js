...
import { isLocalDomain } from '../../internal/smtp/server.js';
...
export default async function messageRoutes(fastify) {
  ...
  fastify.get('/v1/inboxes/:id/messages', {
    ...
    // Skip POP3 fetch for local domains — mail arrives via built-in SMTP server.
    if (fetchNew !== false) {
      try {
        const inbox = await inboxRepo.getInboxById(id);
        const domain = inbox.email_address?.split('@')[1];
        const isDomainLocal = domain && isLocalDomain(domain);
